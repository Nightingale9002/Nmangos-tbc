...
## [机制] 哈兰(Halaa)：商人/卫兵刷点本来就没有 + 易主后"走开再回来就没 NPC" — 2026-09-17 已改（`dev/076` + 代码，待推云）

> ⚠️ **2026-09-18 已整条回退 —— 本章的自制方案作废，请看下一节「哈兰回归原版事件逻辑」。**
> 回退原因（站长报告）："重启后哈兰直接归部落了" + "NPC 刷新位置不对" + "哈兰应该按原来的事件逻辑来，
> 而不是我们写的这个"。本章保留仅为记录踩过的坑与当时的判断依据。
...
## [平衡] 掉率与"超稀有掉落"：2026-09-18 上线（本地验证 → 云端随 01:30 全量重编）

- **站长定案两条**：
  1. **全局品质掉率**（`/opt/mangos/bin/mangosd.conf`，09-18 00:28 已改；备份 `mangosd.conf.bak_droprate_20260918_0028`）：
     `Rate.Drop.Item.Poor = 0.5` / `Normal = 1` / `Uncommon = 2` / `Rare = 5` / `Epic = 3` / `Legendary = 2`；
     `Rate.Drop.Item.Referenced` 保持 **1**；`Rate.Drop.Money`、`Rate.Auction.Cut` **未动**。
     （站长原口径是"灰0.5/白不变/绿3/蓝5/紫5/橙10"，复核后定 **0.5/1/2/5/3/2**：绿 5 倍会让材料与绿装泛滥，紫 5 倍叠加 `Referenced` 引用链会失控。）
  2. **超稀有（万分之一级）世界掉落**：`dev/081_超稀有掉落提升.sql`（幂等）。
- **为什么不用 `Rate.Drop.Item.Referenced` 抬超稀有**：它是**全局均匀乘数**，只作用于引用行外层几率，无法区分"万分之一级"和"普通 1.94%" —— ×10 会让普通引用行 1.94% → 19.4%（材料/绿装全线泛滥，>10% 的直接变必掉），而 0.0005% → 0.005% **依然等于没有**。故只做**按稀有度分档、定向改组**。
- **分档规则（只抬外层组出现率，不动组内容 → 组内物品相对比例不变）**：
  - **A) 紫装组**（`603xx` 全域 + `41301` + `30012`），原 ≤0.1%：≤0.001%→**0.15%**、≤0.01%→**0.25%**、其它→**0.5%**（实测 **2,266** 行 —— 正好覆盖全库所有 ≤0.01% 的引用行）；
  - **B) 配方/图样/设计图组**（`50501/50500/50502/50503/50526/50536/50545/50548/50549/50551/50557/50563/36100/50498/50499`），原 <0.5%：≤0.05%→**0.6%**、其它→**1.2%**（实测 ≈**2,800** 行，主要是 50501 珠宝 1,517 + 50500 666 + 50502 894）。
  - **幂等依据**：改写后的取值全部落在阈值**之上**（A 组 0.15/0.25/0.5 > 0.1；B 组 0.6/1.2 > 0.5）→ 重跑不再命中。本地连续跑两次验证（第二次 rows=0）。
- **明确不动**：>0.5% 的普通引用行 **26,346** 行（材料/绿装/普通引用全在这里，动了就是全服泛滥 + AH 爆量）；`34002/34003/34024` 等外层本就 100% 必掉的技能书/圣契组；**直接掉落**的 ≤0.01% 超稀有物品行约 **2,980** 行（其中大半是宝石，本就有其它常见渠道，抬了收益小、副作用是宝石价崩；若日后要动需另开文件单独评估）。
- **AHBot 挂单时长**：`AuctionHouseBot.Time.Min = 12` / `Time.Max = 12`（`/opt/mangos/etc/ahbot.conf`，09-18 00:30 已改；备份 `ahbot.conf.bak_time_20260918_0030`）。起因："很多挂单时间太长了"。
- **生效方式**：`Rate.*` 与掉落表都是 **mangosd 启动时载入** → 必须重启（本次随 01:30 起的全量重编重启生效）。
- **验收**：`bash /root/verify_20260918.sh`（**v2**）第 3b/3c/4/5 节 —— A/B 两组残留行数应为 0、六项掉率数值、`Time.Min/Max = 12`。

### 补充定案（2026-09-18）：60345 组里"显示 0.1%"的 22 行 —— **有意保留，不修**

- **事实**：`081` A 组用 `ChanceOrQuestChance <= 0.1` 判定，但该列是 **float**，这 22 行实际存的是 `0.10000000149`（> 0.1）→ 被漏掉，仍停在 0.1%。它们**全是太阳井高地（map 580）的炎刃 / 影刃精英**（lvl 70-72），共 **127 个刷点**。
- **站长问"对物品本身的掉率影响有多少" → 实测（云端库）**：
  - 这 21 件世界紫装（`31318`~`31343`，ilvl 100，绑 `60345`）**只有这一个掉落系统**：`reference_loot_template 60345` 内 21 件全是 `chance=0 / groupid=1` → 走 `LootGroup::Roll` 的等概率分支，**抽中就给随机 1 件**。所以「任意一件」= 外层几率（1/1000），「指定某一件」= 外层几率 ÷ 21（1/21,000）。
  - 该组被 **670 个怪模板 / 3,774 个刷点 / 16 张地图**引用，按几率档：**0.15% 档 2,492 刷点**（主要是外域世界怪 map 530 = 1,659）、**0.25% 档 1,155 刷点**（卡拉赞 510 / 祖阿曼 184 / 蒸汽 118 …）、**0.1% 档就是这 127 个刷点（太阳井）**。
  - 静态加权（刷点 × 几率）：太阳井 **0.13**，全组 **6.76** → 太阳井只占 **1.9%**；若改成 0.5% 则占 8.8%，全组总量 **+7.5%**。
  - 再按**刷新速度**加权（外域世界怪 300 秒 vs 副本 2 小时）：太阳井占比降到 **0.23%**。
  - 另查：21 件里 `31331`（夜色之刃）另有容器来源（加固的垃圾箱 `29569`，0.05%）；`31334`（自然愤怒法杖）有一条 `npc_vendor` 记录（商人 `26309` "Weapons Vendor"），但该商人**没有任何刷点**（死数据）。其余掉落表（gameobject/pickpocket/skinning/disenchant/prospecting/fishing/mail）均为 0。
- **定案**：**不改**。理由：它只是该物品众多来源里占 2% 以下的一个口子，改了对"能不能打到"无感；而太阳井小怪是全服被刷得最勤的内容，改成 0.5% 会先让这 21 件世界紫贬值。
- ⚠️ **教训**：判断"某处掉率该不该调"**不能只看那一行的数值**，要算**该物品的全部来源占比**（刷点数 × 几率 × 刷新速度）—— 否则会把一个 2% 的口子当成 100% 的问题来调。同理，`npc_vendor` 里存在**没有刷点的商人**的死数据，排查来源时要顺手确认商人是否真的刷在世界里。

---

## [修复] 哈兰回归原版事件逻辑（2026-09-18，站长定案）

- **站长报告**：重启后哈兰直接归部落；NPC 刷新位置不对；"哈兰应该按原来的事件逻辑来，而不是我们写的这个"。
- **根因（两条，都是我们自己的改动，且"应用时不暴露、一重启才暴露"）**：
  1. **代码侧**：fork 的三个 commit `d540af8b7`（修复哈兰宕机）/ `136f51d10`（修复哈兰重新加载没有npc）/ `a507553c5`（修复哈兰）
     把哈兰改成**自制逻辑** —— 初始归属 = 部落（`m_zoneOwner(HORDE)`）+ 双方 NPC 常驻 DB 刷点 + 易主时 despawn 输方
     → **每次重启哈兰都回到部落手里**。
  2. **数据侧**：`dev/076_补哈兰NPC刷点.sql` 按**初版估值坐标**插入 40 行常驻刷点（guid 9000001-9000040）
     → **位置不是原版位置**（绕哈兰旗帜画的两圈）。
  3. `creature` 表在 mangosd **启动时载入** → 这两条合起来就是"重启后镇里站着部落的常驻 NPC，且全在错误坐标上"。
- **原版逻辑（本次已恢复）**：
  - 初始**中立**（`m_zoneOwner = TEAM_NONE`，墓地也不归属任何一方），镇里**没有常驻 NPC**；
  - 某方占领时 `OutdoorPvPNA::HandleEvent` 对占领事件**返回 false** → 交回 DB 事件脚本，由
    **`dbscripts_on_event` `11503`（部落）/ `11504`（联盟）**在**原版真实坐标**上召唤该阵营的 **5 商人 + 15 卫兵**；
  - 卫兵死亡后按原版逻辑 5 分钟（围城时 1 小时）在原地召唤补充。
- **落地内容**：
  - **代码**：`git checkout 136f51d10^ -- src/game/OutdoorPvP/OutdoorPvPNA.cpp src/game/OutdoorPvP/OutdoorPvPNA.h`
    → 与上游原版**仅差** `d540af8b7` 那 7 行空队列保护（**保留**，它是真宕机修复）；`.h` 与上游**完全一致**。
  - **数据**：`dev/082_哈兰回归原版事件逻辑_删除预置刷点.sql`（删除 guid 段 9000001-9000040 + 关联表防御性清理 + 校验；幂等）。
    `dev/076` 保留在仓库里但**不要在新库上执行**（已被 082 取代）。
  - **本地验证**：编译通过（只重编 `OutdoorPvPNA.cpp` + `OutdoorPvPMgr.cpp` + 链接）；082 应用后
    "残留刷点 = 0 / 镇内哈兰 NPC = 0"，事件脚本 `11503`/`11504` 各 20 行仍在。
- ⚠️ **原版行为的两点"副作用"（站长已知情，属原版行为，不是新 bug）**：
  1. **中立时镇里没有 NPC** —— 要有人占领，对应阵营的商人与卫兵才会出现；
  2. 召唤出来的 NPC 是**临时召唤**，**网格卸载 / 玩家走开再回来会消失**（这正是当初 `136f51d10` 想修的现象，回退后回归原版）。
- **教训（重要）**：改"事件驱动"的玩法前，**先查 `dbscripts_on_*` 里有没有原版脚本** —— 这次 `11503/11504`（各 20 行、
  带真实坐标）**一直都在我们库里**，我们却以为"刷点从来没做过"，于是自造了坐标。
  **原版数据的正确出处**：`_tbc-db_ref\Full_DB\TBCDB_1.11.0_Vengeance_One_A_Cmangos_Story.sql.gz`（gzip，需流式搜索），
  可复用 `_agent_tmp\extract_11503b.py` 抽取任意脚本行。

---

## [待定] 魅惑 vs 30 码"牵引绳"：站长 2026-09-18 定 **「先不改」**（留档）

- **现象**：被魅惑的单位（宠物同源路径）在约 30 码外会触发 `CombatManager` 的 leash 判定 → 强制 EVADE → `UnitAI::EnterEvadeMode()` → `MoveTargetedHome()`，表现出来就是"怪直接跑回家 + 魅惑断掉"。
- **排查结论：全代码里与"距离"有关的只有三条**：
  1. **引导类魅惑的法术射程**（如精神控制）—— 超距离断开**是设计如此**，不是 bug；
  2. `Pet::Update` 在 `GetMap()->GetVisibilityDistance()` 之外解召唤（大陆 **100** 码 / 副本 **170** 码；且还要求 `owner->HasCharm() && !owner->HasCharm(GetObjectGuid())`）；
  3. **`LeashRadius = 30`**（`mangosd.conf`，`World.cpp` 载入）→ 就是上述"跑回家"那条。
- **本 fork 已有现成开关，不必动全局 `LeashRadius`**：`GetCombatManager().SetLeashingDisable(true)` —— 同文件里 `npc_warp_gate` 对被魅惑的邪能火炮就是这么绕开 30 码判定的（见上文 2026-09-16 任务 10911 条目）。**日后若要改，"被魅惑单位 leash 免判"是最小改动。**
- **另注**：`Unit::GetClientControlling()` 对"非占有型魅惑生物"返回 `nullptr` → `Unit::HandleExitCombat()` 会把它们路由到 `AI()->EnterEvadeMode()` 而不是 `CombatStop()`；`IsSpellRemovedOnEvade()` 对 `SPELL_AURA_MOD_CHARM/MOD_POSSESS` 返回 **false**（evade 不剥魅惑光环），但 `EnterEvadeMode()` 里的 `MoveTargetedHome()` 照样会把单位拉走。**这条链路是"魅惑看起来被断"的真实来源。**
- **站长决定**：**本次不改**（2026-09-18），本条目仅作留档 —— 以后有人再提"魅惑距离"，从这里接着往下走即可。
