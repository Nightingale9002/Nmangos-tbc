...
void CombatManager::ShareLeashExtension(std::shared_ptr<time_t> const& ptr)
{
    ...
    if (m_owner->GetVictim() && m_owner->GetVictim()->IsControlledByPlayer())
        sLog.outError("[LEASH] Share receiver=%u ptr=%p val=%ld", m_owner->GetGUIDLow(), (void*)m_lastLeashExtension.get(), (long)*m_lastLeashExtension);
}
...
void CombatManager::UpdateLeashExtension()
{
    ...
    if (m_owner->GetVictim() && m_owner->GetVictim()->IsControlledByPlayer())
        sLog.outError("[LEASH] Update guid=%u ptr=%p val=%ld", m_owner->GetGUIDLow(), (void*)m_lastLeashExtension.get(), (long)*m_lastLeashExtension);
}
