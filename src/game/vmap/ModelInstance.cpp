...
namespace VMAP
{
    ...
    bool ModelInstance::intersectRay(const G3D::Ray& pRay, float& pMaxDist, bool pStopAtFirstHit, bool ignoreM2Model, bool frontFacesOnly) const
    {
        if (!iModel)
        {
#ifdef VMAP_DEBUG
        ...
        bool hit = iModel->IntersectRay(modRay, distance, pStopAtFirstHit, ignoreM2Model, frontFacesOnly);
        if (hit)
        {
            distance *= iScale;
            pMaxDist = distance;
